...
**How to stay in sync:** the file is a manual export, not live data. Regenerate it whenever plugins are added, edited, or unpublished — ask the assistant _"Regenerate data/plugins.json"_ or run the SQL query in the previous version against the backend. The live source of truth remains the `plugins` table at `/plugins` and `/admin?tab=plugins`.
